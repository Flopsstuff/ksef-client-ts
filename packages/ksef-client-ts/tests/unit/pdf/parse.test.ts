...
describe('detectUpoVersion', () => {
  ...
  // The scan is anchored to the document's first element, so a commented-out
  // root cannot decide the version — it matched by name alone before.
  it('ignores a commented-out root tag before the real one', () => {
    const xml =
      '<?xml version="1.0"?>\n' +
      '<!-- <Potwierdzenie xmlns="http://upo.schematy.mf.gov.pl/KSeF/v4-2"> -->\n' +
      '<Potwierdzenie xmlns="http://upo.schematy.mf.gov.pl/KSeF/v4-3"><X>1</X></Potwierdzenie>';
    expect(detectUpoVersion(xml)).toBe('UPO(4.3)');
  });

  // A processing instruction ends at `?>`, not at the next `>`, so its content
  // is free to hold anything — including something that reads like a root tag.
  // Refusing `<?` at the opening bracket alone was not enough: the scan simply
  // found the fake tag further inside the instruction and read its namespace,
  // while the real root passed the `Potwierdzenie` check on the parsed object.
  it('ignores a root tag quoted inside a processing instruction', () => {
    const xml =
      '<?xml version="1.0"?>\n' +
      '<?xml-stylesheet href="<Potwierdzenie xmlns=\'http://upo.schematy.mf.gov.pl/KSeF/v4-2\'>"?>\n' +
      '<Potwierdzenie xmlns="http://upo.schematy.mf.gov.pl/KSeF/v4-3"><X>1</X></Potwierdzenie>';
    expect(detectUpoVersion(xml)).toBe('UPO(4.3)');
  });

  // XML forbids a bare `<` in an attribute value but permits `>`, so a start
  // tag does not end at the first `>` — it ends at the first one outside the
  // quotes. Stopping early truncated the tag and dropped the xmlns after it.
  it('reads a namespace declared after an attribute containing a bare >', () => {
    const xml =
      '<Potwierdzenie note="a > b" xmlns="http://upo.schematy.mf.gov.pl/KSeF/v4-3">' +
      '<X>1</X></Potwierdzenie>';
    expect(detectUpoVersion(xml)).toBe('UPO(4.3)');
  });

  // A DOCTYPE's internal subset is bracketed and may contain `>` of its own,
  // so it ends at the first unquoted `>` at bracket depth zero, not before.
  it('skips a DOCTYPE whose internal subset contains angle brackets', () => {
    const xml =
      '<!DOCTYPE Potwierdzenie [ <!ENTITY x "a > b"> ]>\n' +
      '<Potwierdzenie xmlns="http://upo.schematy.mf.gov.pl/KSeF/v4-2"><X>1</X></Potwierdzenie>';
    expect(detectUpoVersion(xml)).toBe('UPO(4.2)');
  });

  it('survives a comment that mentions no version at all', () => {
    const xml =
