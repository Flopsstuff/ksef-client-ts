...
features:
  - title: Complete API Coverage
    details: KSeF API v2.6.1 — auth, sessions, invoices, permissions, tokens, certificates, QR codes, and more. All types aligned with the official OpenAPI spec.
  ...
  - title: Environment-Agnostic Core
    details: Main entry point is portable across Node, Deno, and browsers. Node.js-only features (filesystem access, XSD validation) are available via the `ksef-client-ts/node` export, keeping the core library small and dependency-free.
---
