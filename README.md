...
- **PDF visualization** — render FA(2)/FA(3) invoices and UPO(4.2)/(4.3) receipts to print-ready PDF offline from version-specific, declarative templates, with Polish/English/bilingual labels and the embedded KSeF Code I verification QR; the renderer touches no filesystem, so the same call runs on Node, in a browser, or on an edge runtime, and `ksef invoice pdf` brings it to shell workflows. Requires the optional `pdfmake` peer (`npm i "pdfmake@^0.2.20"`), so the core install stays dependency-free
...
- **Zero HTTP dependencies** — native `fetch` (Node 20+); dual ESM/CJS via tsup
